...
description: Create directly usable outbound phone-call Agent Skills that bind source and durable result-output contracts at the right level, connect data such as Google Forms, Typeform, Jotform, SurveyMonkey, Tally, TikTok Ads, Notion, Airtable, local CSV, or custom systems to an MCP one-off call provider route, compile per-record call goals, enforce safety rules, and configure source writeback, source-adjacent result artifacts, or new local result CSV output.
---
...
- `typeform`: Typeform form responses via the Typeform API (Bearer personal access token) or signed webhooks.
- `jotform`: Jotform form submissions via the Jotform API (API key) or webhooks.
- `surveymonkey`: SurveyMonkey survey responses via the SurveyMonkey v3 API (OAuth bearer token) or signed webhooks.
- `tally`: Tally form responses via the Tally API (API token) or signed webhooks.
