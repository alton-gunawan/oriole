      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <Card className="p-2 lg:col-span-2">
          {FAQS.map((faq, i) => {
            const isOpen = open === i;
            return (
              <div key={faq.qKey} className="border-b border-zinc-100 last:border-b-0">
                <Collapsible
                  isOpen={isOpen}
                  onOpenChange={(next) => setOpen(next ? i : null)}
                  trigger={
                    <span className="flex w-full items-center justify-between gap-4 px-4 py-4 text-left">
                      <span className="text-sm font-semibold text-zinc-800">{t(faq.qKey)}</span>
                      <IconChevronDown
                ...
                >
                  <p className="px-4 pb-4 text-sm leading-relaxed text-zinc-500">{t(faq.aKey)}</p>
                </Collapsible>
              </div>
            );
          })}
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <a
              ...
              className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold text-zinc-700 transition hover:bg-zinc-50 active:scale-[0.98]"
            >
          ...
          <Card className="bg-zinc-950 p-6">
            ...
            <ul className="mt-3 space-y-2 text-xs text-zinc-400">
              ...
              <li className="flex items-center justify-between">
                ...
                <span className="text-zinc-500">{t('help.waitingConfig')}</span>
              </li>
              <li className="flex items-center justify-between">
